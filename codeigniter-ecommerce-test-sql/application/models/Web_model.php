...
class Web_Model extends CI_Model
{
    ...
    public function get_all_search_product($search)
    {
		...
		$sql = "SELECT *
            FROM tbl_product
            JOIN tbl_category ON tbl_category.id = tbl_product.product_category
            JOIN tbl_brand ON tbl_brand.brand_id = tbl_product.product_brand
            JOIN tbl_user ON tbl_user.user_id = tbl_product.product_author
            WHERE tbl_product.publication_status = 1
            AND (
                tbl_product.product_title LIKE '%" . $search . "%' OR
                tbl_product.product_short_description LIKE '%" . $search . "%' OR
                tbl_product.product_long_description LIKE '%" . $search . "%' OR
                tbl_category.category_name LIKE '%" . $search . "%' OR
